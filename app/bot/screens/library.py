...
class LibraryScreen(Screen):
    ...
    async def _render_main(self, context: Context) -> ScreenRenderResult:
        ...
        if not movies:
            ...
            keyboard = [
                [InlineKeyboardButton(f"🎬 Browse Movies ({len(movies)})", callback_data=LIBRARY_MOVIES)],
                [InlineKeyboardButton("🔄 Scan Library", callback_data=LIBRARY_SCAN)],
                [InlineKeyboardButton("« Back to Menu", callback_data=LIBRARY_MAIN)],
            ]
    ...
    async def _render_list(self, context: Context) -> ScreenRenderResult:
        ...
        if not movies:
            ...
            keyboard = []
            if filter_query:
                keyboard.append([InlineKeyboardButton("❌ Clear Filter", callback_data=LIBRARY_CLEAR_FILTER)])
        ...
        nav_buttons = []
        if page > 0:
            nav_buttons.append(InlineKeyboardButton("« Previous", callback_data=LIBRARY_MOVIES_PREV))
        ...
        filter_buttons = []
        if filter_query:
            filter_buttons.append(InlineKeyboardButton("❌ Clear Filter", callback_data=LIBRARY_CLEAR_FILTER))
    ...
    async def _render_detail(self, context: Context) -> ScreenRenderResult:
        ...
        # Genres
        if movie.genres:
            genres_text = ", ".join([g.capitalize() if isinstance(g, str) else g.value.capitalize() for g in movie.genres])
        ...
        if movie.poster_path and Path(movie.poster_path).exists():
            # TODO: For local poster, we'd need to serve it via a web server
            # For now, skip local posters
            pass
        
        # Check if we have IMDb poster URL in metadata
        if hasattr(movie, 'imdb_id') and movie.imdb_id:
            # The poster_path might actually be a URL from IMDb API
            if isinstance(movie.poster_path, str) and movie.poster_path.startswith('http'):
    ...
    async def _render_delete_confirm(self, context: Context) -> ScreenRenderResult:
        ...
        keyboard = [
            [InlineKeyboardButton("✅ Yes, Delete", callback_data=f"{LIBRARY_CONFIRM_DELETE}{movie.id}")],
    ...
    async def _filter_movies(self, movies, query: str):
        ...
        for movie in movies:
            ...
            # Check genres
            if movie.genres:
                genres_text = " ".join([g.value if hasattr(g, 'value') else str(g) for g in movie.genres]).lower()
    ...
    async def _scan_library(
        ...
        try:
            ...
            context.update_context(
                scan_result=f"✅ Scanned: {movies_count} movies"
            )
